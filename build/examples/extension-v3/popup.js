...
chrome.runtime.onMessage.addListener(({ answer, error }) => {
  if (answer) {
   // Extract loading.port and discharge.port from the answer
   const loadingPortRegex = /(?:'|")?loading\.port(?:'|")?:\s*(?:"(.*?)"|(.*?))/;
   const dischargePortRegex = /(?:'|")?discharge\.port(?:'|")?:\s*(?:"(.*?)"|(.*?))/;

  let loadingPortMatch = answer.match(loadingPortRegex);
  let dischargePortMatch = answer.match(dischargePortRegex);

    console.log("Loading port match:", loadingPortMatch);
    console.log("Discharge port match:", dischargePortMatch);

    if (loadingPortMatch && dischargePortMatch) {
      let loadingPort = loadingPortMatch[1] || loadingPortMatch[2];
      let dischargePort = dischargePortMatch[1] || dischargePortMatch[2];
      ...
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs.length > 0) {
          ...
          chrome.runtime.sendMessage({
            action: "executePuppeteerScript",
            tabId: currentTabId,
            loadingPort: loadingPort,
            dischargePort: dischargePort,
        });
      } else {
        console.error("Could not retrieve the current tab ID.");
      }
    });
  } else {
    console.error("Could not extract loading and discharge ports from the answer.");
  }
